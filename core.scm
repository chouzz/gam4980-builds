...
(define code.sc
  (let* ((macros '((CYCLES n) (set+ executed n)
         ...
         (*ab '(set ea (READ16 pc)
                    pc (+ pc 2)))
         (*abx '(set et (READ16 pc)
                     ea (+ et ix)
                     pc (+ pc 2)))
         (*aby '(set et (READ16 pc)
                     ea (+ et iy)
                     pc (+ pc 2)))
         ...
         (*imm '(set ea pc
                     pc (+ pc 1)))
         (*ind '(set ea (READ16 (READ16 pc))
                     pc (+ pc 1)))
         (*indx '(set ea (READ16 (+ (READ16 pc) ix))
                      pc (+ pc 1)))
         (*rel '(set ea (+ pc 1 (convert-type (READ8 pc) int8_t))
                     pc (+ pc 1)))
         (*xin '(set ea (READ16W (bit-and 0xff (+ (READ8 pc) ix)))
                     pc (+ pc 1)))
         (*yin '(set et (READ16W (READ8 pc))
                     pc (+ pc 1)
                     ea (+ et iy)))
         (*yip `(begin ,*yin ,(page-boundary 'et 'ea)))
         (*zp '(set ea (READ8 pc)
                    pc (+ pc 1)))
         (*zpi '(set ea (READ16W (READ8 pc))
                     pc (+ pc 1)))
         (*zpx '(set ea (bit-and (+ ix (READ8 pc)) 0xff)
                     pc (+ pc 1)))
         (*zpy '(set ea (bit-and (+ iy (READ8 pc)) 0xff)
                     pc (+ pc 1)))
         (*acc '(begin))
         (*imp '(begin))
         (*adcd '(begin
            ...
            (label _exit
                   (if (>= executed cycles)
                       (begin
                         ...
                         (return executed))
                       NEXT))
            (label _next (sc-insert "goto *_table[READ8(pc++)];"))
            ,@(list-tabulate
               256
               (lambda (i)
                 `(begin
